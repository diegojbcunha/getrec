  ...
  try {
    const [
      ...
      db
        .from("presencas")
        .select(
          "*, matriculas(id, colaborador_id, treinamento_id, colaboradores(nome,setor), treinamentos(nome,area))",
        )
        .order("data", { ascending: false }),
      db
        .from("matriculas")
        .select("*, colaboradores(nome,setor), treinamentos(nome,area,status)")
        .order("created_at", { ascending: false }),
      db.from("treinamentos").select("id,nome,area,status").order("nome"),
      db
        .from("colaboradores")
        .select("id,nome,setor,cargo,ativo")
        .order("nome"),
    ]);
...
async function deletePresenca(id) {
  ...
  try {
    const { error } = await db.from("presencas").delete().eq("id", id);
