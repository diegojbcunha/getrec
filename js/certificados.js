  ...
  try {
    const [
      ...
      db
        .from("certificados")
        .select(
          "*, matriculas(*, colaboradores(nome,setor,cargo), treinamentos(nome,area,carga_horaria,instrutor))",
        )
        .order("emitido_em", { ascending: false }),
      db
        .from("matriculas")
        .select(
          "*, colaboradores(nome,setor,cargo), treinamentos(nome,area,carga_horaria,instrutor)",
        )
        .order("created_at", { ascending: false }),
      db.from("treinamentos").select("id,nome,area,status").order("nome"),
      db
        .from("colaboradores")
...
async function emitirCertificados() {
  ...
  try {
    ...
    const { error } = await db.from("certificados").insert(inserts);
...
async function deleteCert(id) {
  if (
    ...
    return;
  try {
    const { error } = await db.from("certificados").delete().eq("id", id);
    if (error) throw error;
