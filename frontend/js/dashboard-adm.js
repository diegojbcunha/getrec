...
async function loadAll() {
  ...
  try {
    const [treinamentos, colaboradores, matriculas, certificados] = await Promise.all([
      api.get("treinamentos?order=created_at&asc=false"),
      api.get("colaboradores?order=created_at&asc=false"),
      api.get("matriculas?select=*,colaboradores(nome,setor),treinamentos(nome,area)&order=created_at&asc=false"),
      api.get("certificados?order=emitido_em&asc=false"),
    ]);
